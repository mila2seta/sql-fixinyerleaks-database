...
-- Query #3 (Difficulty Level: #1)
-- Query Type: Join (Where)
-- Query Type: Aggregate/SUM (Group By)
-- ------------------------------------------------------------
SELECT   TJ.intJobID
		,SUM(TM.monRetailCost * TJM.intQuantity) AS TotalCost
FROM TJobs AS TJ
JOIN TStatuses AS TS
	ON TJ.intStatusID = TS.intStatusID
JOIN TCustomers AS TC
...
WHERE strStatus = 'Complete'
GROUP BY TJ.intJobID
ORDER BY TJ.intJobID

-- Query #6 (Difficulty Level: #1)
-- Query Type: Outer Join (Left)
-- ------------------------------------------------------------
...
-- Query #7 (Difficulty Level: #1)
-- Query Type: Join (Where)
-- Query Type: Distinct
-- ------------------------------------------------------------
SELECT DISTINCT  TC.intCustomerID
		,TC.strFirstName
	    ,TC.strLastName
...
-- Query #8 (Difficulty Level: #1)
-- Query Type: Join (Where)
-- Query Type: Aggregate/Count (Group By)
-- ------------------------------------------------------------
...
-- Query #9 (Difficulty Level: #1)
-- Query Type: Join
-- Query Type: Aggregate/SUM, COUNT (Group By, Having)
-- ------------------------------------------------------------
SELECT TW.intWorkerID
	  ,TW.strFirstName
	  ,TW.strLastName
...
-- Query #13 (Difficulty Level: #1)
-- Query Type: Join
-- Query Type: Aggregate/Count (Group By, Having)
-- ------------------------------------------------------------
SELECT TW.strFirstName
	  ,TW.strLastName
	  ,TC.strFirstName AS CustomerFirstName
	  ,TC.strLastName AS CustomerLastName
...
JOIN TWorkers AS TW
ON TW.intWorkerID = TJW.intWorkerID
GROUP BY TW.strFirstName
		,TW.strLastName
		,TC.strFirstName
		,TC.strLastName
HAVING COUNT(TJ.intJobID) >= 3

-- Query #14 (Difficulty Level: #1)
-- Query Type: Outer Join (Left)
-- Query Type: Aggregate/Count (Group By)
-- ------------------------------------------------------------
SELECT TW.intWorkerID
	  ,TW.strFirstName
	  ,TW.strLastName
	  ,COUNT(TWS.intSkillID) AS TotalSkills
FROM TWorkers AS TW
...
-- Query #15 (Difficulty Level: #1)
-- Query Type: Outer Join (Left)
-- Query Type: Aggregate/Count (Group By)
-- ------------------------------------------------------------
SELECT TS.strService
	  ,COUNT(TJ.intJobID) AS TotalJobs
FROM TServices AS TS
...
SELECT   TJ.intJobID
		...
		,TJM.intQuantity
		,TM.monRetailCost
		,TJM.intQuantity * TM.monRetailCost AS TotalCost
FROM TJobs AS TJ
JOIN TStatuses AS TS
	ON TJ.intStatusID = TS.intStatusID
JOIN TCustomers AS TC
...
WHERE strStatus = 'Complete'
  AND strLastName = 'Nye'
  AND strFirstName = 'Sydney'
ORDER BY TJ.intJobID
		,TM.intMaterialID

-- Query #4 (Difficulty Level: #2)
-- Query Type: Join (Where)
-- Query Type: Aggregate/Count (Group By)
-- ------------------------------------------------------------
SELECT TW.strFirstName
	  ,TW.strLastName
	  ,TW.dtmTerminationDate
	  ,COUNT(TJ.intJobID) AS TotalJobs
FROM TJobs AS TJ
JOIN TJobWorkers AS TJW
	ON TJ.intJobID = TJW.intJobID
...
-- Query #5 (Difficulty Level: #3)
-- Query Type: Join
-- Query Type: Aggregate/SUM, MIN, MAX (Group By)
-- ------------------------------------------------------------
SELECT   TJ.intJobID
		,TJ.strJobDesc
		,TS.strStatus
		,SUM(TJW.intHoursWorked) AS TotalHours
		,MIN(TW.monBillingRate) AS LowestBillingRate
		,MAX(TW.monBillingRate) AS HighestBillingRate
		,SUM(TJW.intHoursWorked * TW.monBillingRate) / SUM(TJW.intHoursWorked) AS AverageBillingRate
FROM TStatuses AS TS
JOIN TJobs AS TJ
	ON TJ.intStatusID = TS.intStatusID
JOIN TJobWorkers AS TJW
